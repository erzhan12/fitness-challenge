...
- Business logic lives in `src/api/services.py`, not in route handlers
- Pydantic models for all request/response schemas in `src/api/models.py`
- Feature plans go in `docs/features/NNNN_PLAN.md` with sequential numbering
- Mock Supabase in tests using `create_mock_query` from `tests/api/conftest.py`
